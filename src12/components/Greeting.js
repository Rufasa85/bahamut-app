...
function Greeting() {
  ...
  const [greeting,setGreeting] = useState("Welcome to class everyone!")
  ...
  const [group, setGroup] = useState(["Sabrina","Frantz","Brett","Carter","Joe","Joe"])
    ...
    <div className="card text-center">
      ...
      <div className="card-body">
        ...
        <ul>
          ...
          {/* <li className="list-group-item">{group[0]}</li>
          <li className="list-group-item">{group[1]} </li>
          <li className="list-group-item">{group[2]}</li> */}
          {group.map((student,i)=><li key={i}>{student}</li>)}
        </ul>
      </div>
    </div>
  );
